...
;(function($) {
	...
	var defaults = {
		...
		// Structure (internal)
		structure: 	 			  '<a href="#" class="btn prev_button"></a>'
	...
	$sk.fn.extend({
		...
		animationCubeStop: function(options)
		{
			...
			for (i = 0; i < total; i++) {
				...
				if($(window).width() > 1920){
					var _vleft 			= (init_left + (width_box * col)) + (this.settings.width_skitter - this.box_skitter.find('.image_main').width() ) / 2;
					var _vleft_image 	= -(width_box * col);
				}else{
					var _vleft 			= (init_left + (width_box * col));
					var _vleft_image 	= -(width_box * col) + (this.settings.width_skitter - this.box_skitter.find('.image_main').width() ) / 2;
				}
		...
		animationHideBars: function(options)
		{
			...
			for (i = 0; i < total; i++) {
				...
				// var _vleft_image = -(width_box * i);
				if($(window).width() > 1920){
                    var _vleft       = width_box * i + (this.settings.width_skitter - this.box_skitter.find('.image_main').width() ) / 2;
                    var _vleft_image = - width_box * i;
                }else{
                    var _vleft       = width_box * i;
                    var _vleft_image = - width_box * i + (this.settings.width_skitter - this.box_skitter.find('.image_main').width() ) / 2;
                }
